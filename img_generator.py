...
def generate_graphs(df: pd.DataFrame):
    ...
    if "Operating System" in df.columns:
        try:
            ...
            texts.append(f"Sistema Operativo: {os_info}")
        except Exception:
            texts.append(f"Sistema Operativo: {df['Operating System'].iloc[0]}")

    if "OS Version" in df.columns:
        try:
            version = float(df["OS Version"].iloc[0])
            texts.append(f"Versión: {version}")
        except:
            texts.append(f"Versión: {df['OS Version'].iloc[0]}")
    ...
    if all(col in df.columns for col in ["Used RAM", "Free RAM", "Available RAM"]):
        try:
            ...
            ax_ram.pie([used, free, available],
                       labels=["Usada", "Libre", "Disponible"],
                       autopct="%1.1f%%", startangle=90)
            ax_ram.set_title("Memoria RAM")
        except ValueError:
            pass
    ...
    if all(col in df.columns for col in ["Total SWAP", "Used SWAP", "Free SWAP"]):
        try:
            ...
            ax_swap.pie([used_swap, free_swap],
                        labels=["Usada", "Libre"],
                        autopct="%1.1f%%", startangle=90)
            ax_swap.set_title("Memoria SWAP")
        except ValueError:
            pass
    ...
    if "Global CPU Usage" in df.columns:
        try:
            ...
            ax_cpu.bar(["Uso CPU"], [cpu_usage], color="orange")
            ax_cpu.set_ylim(0, 100)
            ax_cpu.set_title("Uso Global de CPU (%)")
        except ValueError:
            pass
    ...
    if "Number of CPUs" in df.columns:
        try:
            ...
            texts.append(f"Núcleos de CPU: {cpus}")
        except:
            texts.append("Núcleos de CPU: Desconocido")
    ...
    if "Number of Processes" in df.columns:
        try:
            ...
            texts.append(f"Procesos en ejecución: {process}")
        except:
            texts.append("Procesos en ejecución: Desconocido")
    ...
    if all(col in df.columns for col in ["Disk Total Space", "Disk Available Space"]):
        try:
            ...
            ax_disk.bar(["Total", "Disponible"], [total_disk, available_disk], color=["blue", "green"])
            ax_disk.set_title("Uso de Disco")
        except ValueError as e:
            print(f'{e}')
    
    
    # Network Bar chart with labels
    if all(col in df.columns for col in ["Net Name", "Net Down", "Net Up"]):
        try:
            ...
            # Solo una barra (para un adaptador), con dos secciones apiladas
            ...
            ax_network.set_title("Uso de Red")
            ax_network.set_ylabel("Bytes")
            ax_network.legend(loc="upper right")

            # Etiquetas dentro de las barras (opcional)
    ...
    if texts:
        full_text = "\n".join(texts)
        fig.text(0.01, 0.01, full_text, fontsize=10, va="bottom", ha="left")

    fig.suptitle("Dashboard del Sistema", fontsize=16)
